...
const required=[
  ...
  '<div class="title">Καθημερινά <span class="versionMini">V13.6.2</span></div>',
  "const APP_VERSION='V13.6.2'",
...
if(html.includes('V13.2')||html.includes('V13.3')||html.includes('V13.5.4')||html.includes('V13.5.5')||html.includes('V13.5.8')||html.includes('V13.5.9')||html.includes('V13.6.0')||html.includes('V13.6.1')){console.error('Old version labels remain in index.html'); process.exit(1);}
...
if(!sw.includes('gta-v13-6-2-extended-sessions')){console.error('Service worker cache version missing'); process.exit(1);}
...
if(!html.includes('Phrase from your life today')||!html.includes('🎙 Rehearse')||!html.includes('▶ Play conversation')){console.error('Daily loop polish features missing'); process.exit(1);}
if(!html.includes('Ellinomatheia A1/A2 · import-ready')||!html.includes('function importExamContentJson')){console.error('Exam import-ready loader missing'); process.exit(1);}
if(!html.includes('function playDialogueConversation')||!html.includes('function startDialogueRehearsal')||!html.includes('function recordRehearsalTurn')||!html.includes('speakingNow')||!html.includes('audio/mp4')){console.error('Speaking rehearsal functions/styles missing'); process.exit(1);}
if(!html.includes('dialogueReviewStats().open')){console.error('Open dialogue review count missing'); process.exit(1);}

if(!html.includes('60 min/day')){console.error('60-minute progress effort option missing'); process.exit(1);}
if(!html.includes('return [5,15,30,45,60].includes(v)?v:15')){console.error('Extended adaptive minute options missing'); process.exit(1);} 
if(!html.includes('[5,15,30,45,60].map')){console.error('45/60 time buttons missing'); process.exit(1);} 
if(!html.includes('Deep mixed quiz')||!html.includes('addDialogueRehearsals(minChoice>=60?2:1)')){console.error('Long-session depth logic missing'); process.exit(1);} 

const readme=fs.readFileSync('README.md','utf8');
if(!readme.includes('# Καθημερινά V13.6.2 — Extended Adaptive Study Sessions')){console.error('README heading does not match V13.6.2 extended-sessions build'); process.exit(1);}
if(!html.includes('<div class="title">Καθημερινά <span class="versionMini">V13.6.2</span></div>')){console.error('Visible app header version does not show V13.6.2'); process.exit(1);}
if(!html.includes("const APP_VERSION='V13.6.2'")){console.error('APP_VERSION is not V13.6.2'); process.exit(1);}
if(!html.includes('<div class="integrityItem"><b>V13.6.2</b><span class="muted">Version label</span></div>')){console.error('Build Integrity panel does not show V13.6.2'); process.exit(1);}
...
console.log('GTA V13.6.2 Καθημερινά extended-sessions smoke test passed.');
