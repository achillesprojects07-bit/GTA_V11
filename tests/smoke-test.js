...
const required=[
  ...
  '<div class="title">Καθημερινά <span class="versionMini">V14.14</span></div>',
  ...
  'v1414OpenExamSimulation',
  'v1414StartMock',
  'v1414FinishSection',
  'v1414OpenWeaknessReport',
  'v1414DashboardPanel',
  'Practice mocks by skill',
  'Weak items become repetition work',
  'gta-v14-14-exam-simulation-layer',
...
if(!sw.includes("const CACHE_NAME='gta-v14-14-exam-simulation-layer'")){console.error('Service worker cache mismatch');process.exit(1)}
if(!readme.includes('# Καθημερινά V14.14 — A1/A2 Exam Simulation Layer')){console.error('README heading mismatch');process.exit(1)}
if(!pkg.includes('"version":"14.14.0"')){console.error('Package version mismatch');process.exit(1)}
const forbidden=["APP_VERSION='V14.13';",'<span class="versionMini">V14.13</span>','gta-v14-13-writing-sentence-production-engine'];
...
if(scripts.length<7){console.error('Expected main script plus V14 extension scripts');process.exit(1)}
scripts.forEach((script,i)=>{let f=`/tmp/kathimerina-v14-14-${i}.js`;fs.writeFileSync(f,script);try{execSync(`node --check ${f}`,{stdio:'pipe'});}catch(e){console.error(`SYNTAX ERROR script ${i}:\n`+e.stderr.toString().slice(0,3000));process.exit(1)}});
...
if(html.indexOf('window.v1414OpenExamSimulation')<html.indexOf('V14.13 Writing + Sentence Production Engine')){console.error('V14.14 extension should be after V14.13');process.exit(1)}
console.log('Καθημερινά V14.14 A1/A2 Exam Simulation Layer smoke test passed.');
