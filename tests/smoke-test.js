...
const required=[
  ...
  '<div class="title">Καθημερινά <span class="versionMini">V13.6.30</span></div>',
  "const APP_VERSION='V13.6.30'",
  ...
  'gta-v13-6-30-scores-english-voice'
];
...
if(!sw.includes("const CACHE_NAME='gta-v13-6-30-scores-english-voice'")){console.error('Service worker cache mismatch');process.exit(1)}
if(!readme.includes('# Καθημερινά V13.6.30 — Today Reset + Session Focus')){console.error('README heading mismatch');process.exit(1)}
if(!readme.includes('App header shows `Καθημερινά V13.6.30`')){console.error('README verification mismatch');process.exit(1)}
if(!pkg.includes('"version":"13.6.30"')){console.error('Package version mismatch');process.exit(1)}
const forbidden=['V13.6.24</span>',"APP_VERSION='V13.6.24'",'gta-v13-6-28-two-voice-dialogue','ONE BUTTON PATH ACTIVE','V13.6.22</span>','V13.6.21</span>'];
...
// V13.6.30 regression guard: guided reading must advance the screen, not call an undefined function.
(function(){
  ...
  if(body.indexOf('renderHome()')===-1 && body.indexOf('showGuidedScore')===-1){console.error('gradeGuidedReading does not repaint (needs renderHome or showGuidedScore)');process.exit(1);}
  if(html.indexOf('function continueGuidedAfterScore(')!==-1 && html.slice(html.indexOf('function continueGuidedAfterScore(')).indexOf('renderHome()')>400){console.error('continueGuidedAfterScore must call renderHome to advance');process.exit(1);}
})();

console.log('GTA V13.6.30 Καθημερινά Today Reset + Session Focus smoke test passed.');
