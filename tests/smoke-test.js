...
const required=[
  ...
  '<div class="title">Καθημερινά <span class="versionMini">V13.6.22</span></div>',
  "const APP_VERSION='V13.6.22'",
  ...
  'gta-v13-6-22-clear-progressive-path'
];
...
if(!sw.includes("const CACHE_NAME='gta-v13-6-22-clear-progressive-path'")){console.error('Service worker cache mismatch');process.exit(1)}
if(!readme.includes('# Καθημερινά V13.6.22 — Clear Progressive Learning Path')){console.error('README heading mismatch');process.exit(1)}
if(!readme.includes('App header shows `Καθημερινά V13.6.22`')){console.error('README verification section mismatch');process.exit(1)}
if(!pkg.includes('"version":"13.6.22"')){console.error('Package version mismatch');process.exit(1)}
...
fs.writeFileSync('/tmp/kathimerina-v13622.js',script);
require('child_process').execFileSync(process.execPath,['--check','/tmp/kathimerina-v13622.js'],{stdio:'inherit'});
...
console.log('GTA V13.6.22 Καθημερινά Clear Progressive Learning Path smoke test passed.');
