...
const required=[
  ...
  '<div class="title">Καθημερινά <span class="versionMini">V13.6.28</span></div>',
  "const APP_VERSION='V13.6.28'",
  ...
  'gta-v13-6-28-two-voice-dialogue'
];
...
if(!sw.includes("const CACHE_NAME='gta-v13-6-28-two-voice-dialogue'")){console.error('Service worker cache mismatch');process.exit(1)}
if(!readme.includes('# Καθημερινά V13.6.28 — Today Reset + Session Focus')){console.error('README heading mismatch');process.exit(1)}
if(!readme.includes('App header shows `Καθημερινά V13.6.28`')){console.error('README verification mismatch');process.exit(1)}
if(!pkg.includes('"version":"13.6.28"')){console.error('Package version mismatch');process.exit(1)}
...
// V13.6.28 regression guard: guided reading must advance the screen, not call an undefined function.
(function(){
...
console.log('GTA V13.6.28 Καθημερινά Today Reset + Session Focus smoke test passed.');
