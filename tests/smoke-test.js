...
const required=[
  ...
  '<div class="title">Καθημερινά <span class="versionMini">V13.6.36</span></div>',
  "const APP_VERSION='V13.6.36'",
  ...
  'English → Greek',
  'xlateGreek',
  'gta-v13-6-36-alphabet-sound-drill'
];
...
if(!sw.includes("const CACHE_NAME='gta-v13-6-36-alphabet-sound-drill'")){console.error('Service worker cache mismatch');process.exit(1)}
if(!readme.includes('# Καθημερινά V13.6.36 — Alphabet Sound Drill')){console.error('README heading mismatch');process.exit(1)}
if(!pkg.includes('"version":"13.6.36"')){console.error('Package version mismatch');process.exit(1)}
...
fs.writeFileSync('/tmp/kathimerina-v13635.js',script);
try{execSync('node --check /tmp/kathimerina-v13635.js',{stdio:'pipe'})}catch(e){console.error('SYNTAX ERROR:\n'+e.stderr.toString().slice(0,1000));process.exit(1)}
console.log('GTA V13.6.36 Alphabet Sound Drill smoke test passed.');
