...
const required=[
  ...
  '<div class="title">Καθημερινά <span class="versionMini">V14.13</span></div>',
  ...
  'v1413StartWriting',
  'v1413SubmitWriting',
  'v1413MakeWritingQuestions',
  'sentenceBuild',
  'sentenceRepair',
  'controlledShortWriting',
  'writing correction queue',
  'gta-v14-13-writing-sentence-production-engine',
  'c0_vowels',
  "unit:'L0.01'",
  "id:'c0_gavros'",
  "unit:'L0.10'"
...
if(!sw.includes("const CACHE_NAME='gta-v14-13-writing-sentence-production-engine'")){console.error('Service worker cache mismatch');process.exit(1)}
if(!readme.includes('# Καθημερινά V14.13 — Writing + Sentence Production Engine')){console.error('README heading mismatch');process.exit(1)}
if(!pkg.includes('"version":"14.13.0"')){console.error('Package version mismatch');process.exit(1)}
const forbidden=["APP_VERSION='V14.12';",'<span class="versionMini">V14.12</span>','gta-v14-12-tutor-partner-correction-mode'];
...
if(scripts.length<6){console.error('Expected main script plus V14 extension scripts');process.exit(1)}
scripts.forEach((script,i)=>{let f=`/tmp/kathimerina-v14-13-${i}.js`;fs.writeFileSync(f,script);try{execSync(`node --check ${f}`,{stdio:'pipe'});}catch(e){console.error(`SYNTAX ERROR script ${i}:\n`+e.stderr.toString().slice(0,3000));process.exit(1)}});
...
if(html.indexOf('window.v1413StartWriting')<html.indexOf('V14.12 Tutor / Partner Correction Mode')){console.error('V14.13 extension should be after V14.12');process.exit(1)}
console.log('Καθημερινά V14.13 Writing + Sentence Production Engine smoke test passed.');
