...
const required=[
  ...
  '<span class="versionMini">V15.5</span>',
  "const APP_VERSION='V15.5'",
  "const LS='gta_v12_state'",
  'V15.5 MASTERY SYSTEM AUDIT',
  'V15.5 Mastery System Audit',
  'v155AuditData',
  'v155AuditPanel',
...
if(!sw.includes("const CACHE_NAME='gta-v15-5-mastery-audit'")){console.error('Service worker cache mismatch');process.exit(1)}
if(!sw.includes("cache:'no-store'")){console.error('SW should use no-store fetches');process.exit(1)}
if(!readme.includes('# Καθημερινά V15.5')){console.error('README heading mismatch');process.exit(1)}
if(!pkg.includes('"version":"15.5.0"')){console.error('Package version mismatch');process.exit(1)}
if(!manifest.includes('./index.html?v=15.5')){console.error('Manifest start_url mismatch');process.exit(1)}
...
scripts.forEach((script,i)=>{let f=`/tmp/kathimerina-v15-5-${i}.js`;fs.writeFileSync(f,script);try{execSync(`node --check ${f}`,{stdio:'pipe'});}catch(e){console.error(`SYNTAX ERROR script ${i}:\n`+e.stderr.toString().slice(0,3000));process.exit(1)}});
...
console.log('Καθημερινά V15.5 Mastery System Audit smoke test passed.');
