...
const required=[
  ...
  '<div class="title">Καθημερινά <span class="versionMini">V13.6.13</span></div>',
  "const APP_VERSION='V13.6.13'",
  ...
  'Grammar/listening/speaking injected into Today',
  'gta-v13-6-13-six-month-plan'
];
...
if(!sw.includes("const CACHE_NAME='gta-v13-6-13-six-month-plan'")){console.error('Service worker cache mismatch');process.exit(1)}
if(!readme.includes('# Καθημερινά V13.6.13 — Six-Month Speaking + Listening Plan')){console.error('README heading mismatch');process.exit(1)}
if(!pkg.includes('"version":"13.6.13"')){console.error('Package version mismatch');process.exit(1)}
const forbidden=['V13.6.12</span>','APP_VERSION=\'V13.6.12\'','gta-v13-6-12-grammar-forecast','V13.6.10</span>','gta-v13-6-10','V13.6.9</span>','gta-v13-6-9'];
...
fs.writeFileSync('/tmp/kathimerina-v13613.js',script);
require('child_process').execFileSync(process.execPath,['--check','/tmp/kathimerina-v13613.js'],{stdio:'inherit'});
...
['sixMonthConfidencePanel()','dailyPartnerMissionCard()','adaptiveSessionPanel()','todayCoverageLine()'].forEach(x=>{if(!home.includes(x)){console.error('Home missing '+x);process.exit(1)}});
...
console.log('GTA V13.6.13 Καθημερινά six-month speaking + listening plan smoke test passed.');
